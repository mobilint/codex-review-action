...
MAX_FILES="${INPUT_MAX_FILES:-500}"
MAX_DIFF_CHARS="${INPUT_MAX_DIFF_CHARS:-1000000}"
...
fetch_pr_and_checkout() {
  ...
  if [[ -z "${MERGE_BASE}" ]]; then
    ...
    git -C "${REPO_DIR}" checkout base >/dev/null 2>&1 || {
      echo "[ERROR] failed to checkout base branch before full-history retry" >&2
      exit 1
    }
...
resolve_context
echo "[INFO] repo=${REPO} pr=${PR_NUMBER} event=${EVENT_NAME} mode=${MODE} sandbox=${SANDBOX_MODE} allow_unsafe_no_sandbox_fallback=${ALLOW_UNSAFE_NO_SANDBOX_FALLBACK}"
require_commands
check_codex_sandbox_support
fetch_pr_and_checkout
prepare_review_assets
